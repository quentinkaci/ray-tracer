...
static void add_island(double xx, double zz, Scene& scene)
{

    PerlinNoise perlin;
    Blob        blob(Point3(-2 + xx, -2, 2 + zz),
              0.1,
              10,
              400.,
              std::make_shared<UniformTexture>(TextureMaterialCaracteristics{
                  0.005, 0.30, 1, 0.05, Color(100, 100, 100)}));
...
int main(int, char*[])
{
    ...
    //     std::make_shared<PointLight>(Point3(0, 90, 0), Color(100, 100,
    //     100)));
    scene.light_sources.emplace_back(std::make_shared<DirectionalLight>(
        Color(100, 100, 100), Vector3(0, -1, 0)));
    scene.light_sources.emplace_back(std::make_shared<DirectionalLight>(
        Color(100, 100, 100), Vector3(0, 0, 1)));

    add_island(0, 0, scene);
    // add_island(-2, 8, scene);
    ...
    // const auto sky = create_image("../textures/cannon.png");
    // sky->load("../textures/cannon.png");
